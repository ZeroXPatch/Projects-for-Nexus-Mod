...
### One-command build + zip (helpful if you just want the DLL)

```bash
cd PetBondMod
GAME_PATH="/path/to/Stardew Valley" ./build-release.sh
```

That script compiles the mod and zips the release output to `PetBondMod/publish/PetBondMod-Release.zip`, which you can drop straight into `Stardew Valley/Mods/PetBondMod/`.
