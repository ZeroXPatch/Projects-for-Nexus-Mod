...
public class ModEntry : Mod
{
    ...
    private void ApplyBobberTweaks(BobberBar bar)
    {
        ...
        if (this.Config.CatchTreasure)
        {
            ...
            if (whichFishField?.GetValue(bar) is int whichFish && LegendaryFishIds.Contains(whichFish))
            {
                isLegendary = true;
            }
            ...
            if (!isLegendary)
            {
                ...
                var progressField = bar.GetType().GetField("distanceFromCatching", ReflectionFlags);

                if (treasureField?.GetValue(bar) is bool treasure && treasure)
                {
                    float progress = 0f;
                    if (progressField?.GetValue(bar) is float currentProgress)
    ...
    private void TryHandleAutoLoot()
    {
        ...
        foreach (Debris debris in Game1.currentLocation.debris.ToList())
        {
            ...
            {
                isFishOrTrash = obj.Category == FishCategory || obj.Category == TrashCategory || obj.HasContextTag("fish_item") || obj.HasContextTag("trash_item");
            }

            bool shouldCollect = (isFishOrTrash && this.Config.AutoLootFishAndTrash) || (!isFishOrTrash && this.Config.AutoLootTreasure);
            if (!shouldCollect)
                continue;
