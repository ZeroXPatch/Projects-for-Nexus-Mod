...
public sealed class PlannerMenu : IClickableMenu
{
    ...
    // Layout tuning (matches your example + prevents overlap with bottom text)
    ...
    )
    {
        ...
        // Build rows first (so we can compute required height cleanly).
        ...
        // ---- Size/position (based on your example) ----
        width = 860;

        // Ensure rows never collide with the bottom info + buttons.
        // Condition: (height - 190) > (RowsTopOffset + (rows-1)*RowStep + RowHeight) + padding
        int rowsBlockBottom = RowsTopOffset + ((_rows.Count - 1) * RowStep) + RowHeight;
        int minHeight = Math.Max(720, rowsBlockBottom + 220); // 220 leaves room for bill text + buttons

        // Clamp to screen (avoid going off-screen on smaller resolutions)
        int maxW = Math.Max(400, Game1.uiViewport.Width - 40);
        int maxH = Math.Max(400, Game1.uiViewport.Height - 40);
        width = Math.Min(width, maxW);
        height = Math.Min(minHeight, maxH);

        xPositionOnScreen = (Game1.uiViewport.Width - width) / 2;
        yPositionOnScreen = (Game1.uiViewport.Height - height) / 2;

        // Place work-hours panel to the RIGHT of the task rows (same logic as your example)
        ...
        // Bottom buttons (matches example)
    ...
    public override void draw(SpriteBatch b)
    {
        ...
        for (int i = 0; i < _rows.Count; i++)
        {
            ...
            // Slightly higher baseline (because row spacing is tighter)
            ...
            // Cost text on the right (always show for paid rows, even if disabled)
            if (row.ShowCost)
            {
        ...
        // Bottom info text (positioned using the buttons so it never overlaps)
        int labelX = xPositionOnScreen + 64;

        // Put 3 lines directly above the buttons, with a clean gap.
        int infoTop = _saveRect.Y - 92;      // start of the 3-line block
        int infoLine = 28;

        // If rows ever get too close, push the info down by increasing menu height via minHeight in ctor.
        ...
        Game1.spriteBatch.DrawString(Game1.smallFont, bill, new Vector2(labelX, infoTop), Color.Black);
        Game1.spriteBatch.DrawString(Game1.smallFont, hint, new Vector2(labelX, infoTop + infoLine), Color.Gray);
        Game1.spriteBatch.DrawString(Game1.smallFont, openWith, new Vector2(labelX, infoTop + (infoLine * 2)), Color.DarkSlateBlue);
