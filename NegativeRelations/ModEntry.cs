    ...
    public class ModEntry : Mod
    {
        ...
        private void TryDoBarks()
        {
            ...
            foreach (NPC npc in Game1.currentLocation.characters.OfType<NPC>())
            {
                ...
                if (this.barkCooldowns.TryGetValue(npc.Name, out double until) && nowMs < until)
                {
                    continue;
                }
                ...
                double cooldown = this.Config.BarkCooldownMinutes * 700; // ~700ms per in-game minute
                this.barkCooldowns[npc.Name] = nowMs + cooldown;
            }
        }
        ...
        private void DrawNegativeHearts(SocialPage socialPage, SpriteBatch spriteBatch)
        {
            ...
            foreach ((NPC npc, Rectangle bounds) in this.GetSocialEntries(socialPage))
            {
        ...
        private IEnumerable<(NPC npc, Rectangle bounds)> GetSocialEntries(SocialPage socialPage)
        {
            List<(NPC, Rectangle)> results = new();
            foreach (string fieldName in new[] { "socialEntries", "_socialEntries", "friends", "slots" })
            {
