...
namespace AquaticLife
{
    public class ModEntry : Mod
    {
        ...
        internal static readonly PerScreen<FishManager> FishManagers = new();
        ...
        private void OnGameLaunched(object? sender, GameLaunchedEventArgs e)
        {
            ...
            configMenu.AddNumberOption(ModManifest, () => Config.FishOpacity, val => Config.FishOpacity = val, name: () => Helper.Translation.Get("config.opacity.name"), tooltip: () => Helper.Translation.Get("config.opacity.tooltip"), min: 0.1f, max: 1.0f);
            configMenu.AddNumberOption(ModManifest, () => Config.FishScale, val => Config.FishScale = val, name: () => Helper.Translation.Get("config.scale.name"), tooltip: () => Helper.Translation.Get("config.scale.tooltip"), min: 0.5f, max: 2.0f);
        ...
        private void OnSaveLoaded(object? sender, SaveLoadedEventArgs e) { FishManagers.Value = new FishManager(Helper); }
        private void OnWarped(object? sender, WarpedEventArgs e) { if (e.IsLocalPlayer) FishManagers.Value?.ChangeLocation(e.NewLocation); }
        private void OnUpdateTicked(object? sender, UpdateTickedEventArgs e) { if (Context.IsWorldReady) FishManagers.Value?.Update(e); }
    }
}