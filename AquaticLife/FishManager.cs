...
namespace AquaticLife
{
    public class FishManager
    {
        ...
        public void Update(UpdateTickedEventArgs e)
        {
            ...
            {
                if (isNight) _fish[i].StartDespawn();
                _fish[i].Update(Game1.currentGameTime);
                if (_fish[i].IsDead)
                {
                    _fish[i].Cleanup();
                    _fish.RemoveAt(i);
            ...
            if (!isNight)
            {
                bool aggressive = _fish.Count < 5;
                uint tickRate = aggressive ? 10u : 60u;
                if (e.IsMultipleOf(tickRate) && _fish.Count < _currentSessionCap)
                {
        ...
        private void TrySpawnFish(bool forceRandomMapPosition)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (IsSafeSpawn(x, y))
                {
                    ...
                    _fish.Add(new FishBoid(candidatePos, randomFishId, _currentLocation));
                    return;
                }
            }
        }
        ...
        public void Draw(SpriteBatch b)
        {
            ...
            foreach (var f in _fish)
            {
                ...
                if (f.Position.X + 128 < viewport.X || f.Position.X - 128 > viewport.X + viewport.Width ||
                    f.Position.Y + 128 < viewport.Y || f.Position.Y - 128 > viewport.Y + viewport.Height)
                    continue;
                f.Draw(b);
            }
        }
