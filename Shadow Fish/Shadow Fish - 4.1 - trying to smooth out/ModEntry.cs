...
namespace ShadowsOfTheDeep
{
    public class ModEntry : Mod
    {
        ...
        private void OnGameLaunched(object? sender, GameLaunchedEventArgs e)
        {
            ...
            configMenu.AddBoolOption(ModManifest, () => Config.ShowVisibleFish, val => Config.ShowVisibleFish = val, name: () => Helper.Translation.Get("config.visible.name"), tooltip: () => Helper.Translation.Get("config.visible.tooltip"));
            configMenu.AddNumberOption(ModManifest, () => Config.MinFishScale, val => Config.MinFishScale = val, name: () => Helper.Translation.Get("config.min-scale.name"), tooltip: () => Helper.Translation.Get("config.min-scale.tooltip"), min: 0.1f, max: 3.0f, interval: 0.1f);
            configMenu.AddNumberOption(ModManifest, () => Config.MaxFishScale, val => Config.MaxFishScale = val, name: () => Helper.Translation.Get("config.max-scale.name"), tooltip: () => Helper.Translation.Get("config.max-scale.tooltip"), min: 0.1f, max: 3.0f, interval: 0.1f);

            // ROTATION
            configMenu.AddNumberOption(ModManifest, () => Config.ModdedFishRotation, val => Config.ModdedFishRotation = val,
                name: () => Helper.Translation.Get("config.rotation.name"),
                tooltip: () => Helper.Translation.Get("config.rotation.tooltip"),
                min: -180f, max: 180f);
            ...
            configMenu.AddBoolOption(ModManifest, () => Config.AvoidCrowding, val => Config.AvoidCrowding = val, name: () => Helper.Translation.Get("config.crowding.name"), tooltip: () => Helper.Translation.Get("config.crowding.tooltip"));
            ...
            configMenu.AddNumberOption(ModManifest, () => Config.MinLifespanSeconds, val => Config.MinLifespanSeconds = val, name: () => Helper.Translation.Get("config.lifespan-min.name"), tooltip: () => Helper.Translation.Get("config.lifespan-min.tooltip"), min: 30f, max: 600f);
            configMenu.AddNumberOption(ModManifest, () => Config.MaxLifespanSeconds, val => Config.MaxLifespanSeconds = val, name: () => Helper.Translation.Get("config.lifespan-max.name"), tooltip: () => Helper.Translation.Get("config.lifespan-max.tooltip"), min: 30f, max: 600f);
            ...
            configMenu.AddNumberOption(ModManifest, () => Config.DensityCapMultiplier, val => Config.DensityCapMultiplier = val, name: () => Helper.Translation.Get("config.cap-mult.name"), tooltip: () => Helper.Translation.Get("config.cap-mult.tooltip"), min: 1.0f, max: 50.0f);
            configMenu.AddNumberOption(ModManifest, () => Config.InitialSpawnChance, val => Config.InitialSpawnChance = val, name: () => Helper.Translation.Get("config.initial-spawn.name"), tooltip: () => Helper.Translation.Get("config.initial-spawn.tooltip"), min: 0.0f, max: 1.0f);
