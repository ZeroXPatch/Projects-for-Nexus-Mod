...
namespace ShadowsOfTheDeep
{
    public class ShadowBoid
    {
        ...
        private const float DragConstantSwim = 0.96f;
        ...
        public ShadowBoid(Vector2 startPos, string itemId, GameLocation location)
        {
            ...
            // LIFESPAN: Random time between min and max, converted to frames (60fps)
            float minLifespan = ModEntry.Config.MinLifespanSeconds;
            float maxLifespan = ModEntry.Config.MaxLifespanSeconds;
            _lifespan = (minLifespan + ((float)Game1.random.NextDouble() * (maxLifespan - minLifespan))) * 60f;
            _currentAge = 0f;
            ...
            // Use configurable min/max scale for size variation
            float minScale = ModEntry.Config.MinFishScale;
            float maxScale = ModEntry.Config.MaxFishScale;
            _scaleVariation = minScale + ((float)Game1.random.NextDouble() * (maxScale - minScale));
        ...
        public void Update(GameTime time, List<ShadowBoid> allShadows)
        {
            ...
            // LIFESPAN CHECK: If fish has reached its lifespan, start despawning
            _currentAge += 1f;
            if (_currentAge >= _lifespan && _lifeState == LifeState.Active)
            {
                StartDespawn();
                return;
            }
            ...
            // Use higher drag for constant swimmers to prevent them from going too fast
            float currentDrag;
            if (_isConstantSwimmer)
            {
                currentDrag = DragConstantSwim; // Slightly higher drag for smoother constant speed
            }
            else
            {
                currentDrag = (_swimState == SwimState.Idle) ? DragIdle : DragWater;
            }
        ...
        private void UpdateSwimBehavior()
        {
            ...
            // CONSTANT SWIMMERS: Maintain smooth continuous swimming
            if (_isConstantSwimmer)
            {
                // Apply constant gentle force to maintain steady speed
                Vector2 force = new Vector2((float)Math.Cos(_wanderAngle), (float)Math.Sin(_wanderAngle));
                _acceleration += force * 0.03f * _individualSpeedFactor;

                // Gentle random direction changes for natural movement
                if (Game1.random.NextDouble() < 0.02f) // 2% chance per frame
                {
                    _wanderAngle += (float)(Game1.random.NextDouble() - 0.5) * 0.3f;
                }

                return; // Skip state machine entirely for constant swimmers
            }

            // REGULAR FISH: Use normal state machine
            switch (_swimState)
            {
                case SwimState.Idle:
                    if (_isInanimate)
                ...
                case SwimState.Gliding:
                    ...
                    else if (_stateTimer <= 0 && _isBurster)
                    {
                        SwitchState(SwimState.Accelerating);
                    }
                    break;
        ...
        public void Draw(SpriteBatch b)
        {
            ...
            // Simplified: No global scale multiplier, just base scale and individual variation
            float globalScale = BaseScale * _scaleVariation;
            ...
            // Choose color based on config - White shows full color, Black shows as shadow
            Color drawColor = ModEntry.Config.ShowVisibleFish ? Color.White : Color.Black;

            // DRAW FISH (colored or shadow)
            b.Draw(_texture, screenPos, _sourceRect, drawColor * finalOpacity,
