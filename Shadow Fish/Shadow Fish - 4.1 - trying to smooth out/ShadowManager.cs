...
namespace ShadowsOfTheDeep
{
    public class ShadowManager
    {
        ...
        private bool _isSmallWaterBody = false;

        // Locations that have too much water and need reduced fish spawns
        private static readonly Dictionary<string, float> LocationCapMultipliers = new()
        {
            { "Custom_ShearwaterBridge", 0.5f },  // 50% reduction
            // Add more problematic locations here if needed
            // { "SomeOtherLocation", 0.3f },
        };
        ...
        public void ChangeLocation(GameLocation location)
        {
            ...
            // Detect small water bodies (fish ponds, small pools)
            _isSmallWaterBody = IsSmallWaterBody(location, waterTiles);

            int densityCap;
            if (_isSmallWaterBody)
            {
                // For small water bodies like fish ponds, use a much higher density
                // and ensure a minimum of 15-30 fish
                densityCap = (int)(waterTiles * ModEntry.Config.SpawnChance * ModEntry.Config.DensityCapMultiplier * 3.0f);
                densityCap = Math.Max(densityCap, 15);
            }
            else
            {
                // Normal density calculation for large bodies of water
                densityCap = (int)(waterTiles * ModEntry.Config.SpawnChance * ModEntry.Config.DensityCapMultiplier);
                if (waterTiles > 0) densityCap = Math.Max(densityCap, 5);
            }

            // Apply location-specific cap multipliers for problematic locations
            if (LocationCapMultipliers.TryGetValue(location.Name, out float multiplier))
            {
                densityCap = (int)(densityCap * multiplier);
                ModEntry.ModMonitor.Log($"Applied {multiplier:P0} cap reduction to {location.Name}. New cap: {densityCap}", LogLevel.Debug);
            }
        ...
        private bool IsSmallWaterBody(GameLocation location, int waterTiles)
        {
            // Detect by location name patterns
            string locationName = location.Name ?? "";

            // Fish pond detection
            if (locationName.Contains("Pond") || locationName.Contains("FishPond"))
            {
                return true;
            }

            // Check for small indoor locations (cabins, sheds with aquariums, etc.)
            if (location.IsOutdoors == false && waterTiles > 0 && waterTiles < 100)
            {
                return true;
            }

            // Any water body with less than 50 tiles is considered small
            if (waterTiles > 0 && waterTiles < 50)
            {
                return true;
            }
        ...
        private void TrySpawnFish(bool forceRandomMapPosition, int retryCount)
        {
            ...
            for (int i = 0; i < retryCount; i++)
            {
                ...
                // Use relaxed spawn rules for small water bodies
                bool isValid = _isSmallWaterBody ?
                    IsSafeSpawnRelaxed(x, y) :
                    IsSafeSpawn(x, y);
        ...
        private bool IsSafeSpawn(int tileX, int tileY)
        {
            ...
            // Require all 8 surrounding tiles to be water
        ...
        private bool IsSafeSpawnRelaxed(int tileX, int tileY)
        {
            if (!IsTileWater(tileX, tileY)) return false;

            // For small ponds, just require the tile itself to be water
            // and at least 2 adjacent tiles to be water (more forgiving)
            int waterNeighbors = 0;
            if (IsTileWater(tileX + 1, tileY)) waterNeighbors++;
            if (IsTileWater(tileX - 1, tileY)) waterNeighbors++;
            if (IsTileWater(tileX, tileY + 1)) waterNeighbors++;
            if (IsTileWater(tileX, tileY - 1)) waterNeighbors++;
