...
namespace ShadowsOfTheDeep
{
    public class ShadowBoid
    {
        ...
        public void Update(GameTime time)
        {
            ...
            // If the steering failed (e.g. spawned in a wall or moving too fast), bounce.
            if (IsPointWater(nextPos))
            {
                ...
                if (!_isInanimate)
                {
                    if (_velocity.LengthSquared() > 0.02f)
                    {
                        ...
                        _rotation = LerpRadians(_rotation, targetRot, 0.1f);
                    }
                }
                else
                {
                    // Gentle spin for debris
        ...
        {
            if (_isConstantSwimmer && newState == SwimState.Idle)
                newState = SwimState.Accelerating;
        ...
        private Vector2 GetAvoidanceSteering()
        {
            ...
            // UPDATE: Increased base lookAhead from 24f to 30f to catch beach corners earlier
            float lookAhead = _isInanimate ? 24f : 30f + (currentSpeed * 25f);

            Vector2 forward = _velocity;
            if (forward == Vector2.Zero)
                forward = new Vector2((float)Math.Cos(_rotation), (float)Math.Sin(_rotation));
            forward.Normalize();
        ...
        private bool IsWall(Vector2 pos)
        {
            ...
            {
                if (_location.doesTileHaveProperty(tileX, tileY, "Passable", "Buildings") == null)
                    return true;
            }
            ...
            if (pos.X < 32 || pos.Y < 32 ||
                ...
                pos.Y > _location.Map.Layers[0].LayerHeight * 64 - 32)
                return true;

            // 4. Detailed Padding (Cardinal AND Diagonal)
            float padding = 20f;
            float offX = pos.X % 64;
            float offY = pos.Y % 64;
            ...
            // RESTORED: Diagonal (Corner) Checks for Beach
