...
namespace ShadowsOfTheDeep
{
    public class ShadowManager
    {
        ...
        public void ChangeLocation(GameLocation location)
        {
            ...
            // 1. Determine User's Requested Range
            ...
            // 2. Calculate Water Surface Area
            int waterTiles = CountWaterTiles(location);

            // 3. Calculate Density Cap
            // Base Formula: tiles * chance
            // Multiplier: Allows user to say "I want 5x the recommended density"
            // If Multiplier is high, this cap essentially does nothing, allowing the User Desired Cap to take over.
            int densityCap = (int)(waterTiles * ModEntry.Config.SpawnChance * ModEntry.Config.DensityCapMultiplier);

            // Ensure tiny ponds still get a few fish if there is at least some water
            ...
            if (_possibleFishIds.Any() && _currentSessionCap > 0)
            {
                ...
                {
                    // Use higher retry count (10) for initial spawn because map-wide random picking hits land often
